...
import { Console, PPLB, FS } from '../util';
...
export function BCMSMostMediaHandler(
  ...
  const advancedFs = createFS({
    base: path.join(process.cwd(), ...config.media.output.split('/')),
  });
        ...
        await PPLB.manage<MediaResponse>(
          ...
          async (data, chunkId) => {
            if (data.data.type !== MediaType.DIR) {
              const dirPath = data.data.isInRoot
                ? []
                : data.data.path.startsWith('/')
                ? data.data.path.split('/').slice(1)
                : data.data.path.split('/');
              const filePath = [...dirPath, data.data.name];
              if (await advancedFs.exist(filePath)) {
                cnsl.info(chunkId, `Removing root file: ${filePath.join('/')}`);
                await advancedFs.deleteFile(filePath);
              }
              const fileNameParts = data.data.name.split('.');
              const fileNameBase = fileNameParts
                .slice(0, fileNameParts.length - 1)
                .join('.');
              const dirFiles = (await advancedFs.readdir(dirPath)).filter((e) =>
                e.startsWith(fileNameBase),
              );
              for (let i = 0; i < dirFiles.length; i++) {
                const dirFile = dirFiles[i];
                const fullPath = [...dirPath, dirFile];
                cnsl.info(chunkId, `Removing file: ${fullPath.join('/')}`);
                await advancedFs.deleteFile(fullPath);
              }
            ...
            try {
              await ChildProcess.advancedExec(
                [
                  ...
                  '--media-config',
                  Buffer.from(JSON.stringify(config.media)).toString('hex'),
                ],
                {
                  onChunk: (type, chunk) => {
